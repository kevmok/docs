...
<CardGroup cols={2}>
  ...
  <Card title="Quickstart #1 Complete" icon="check">
    Completed [Quickstart #1: Basic Task Agent](/docs/quickstart-basic-task-agent).
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Set up Actions">
    Set up **2 (HTTP)[Actions](/docs/actions)**: one to fetch historical PRs, and another to Submit a PR.
  </Step>
...
<Steps>
  <Step title="Go to Purple API Tool">
...
<Steps>
  ...
  <Step title="Add Slots">
    Add the FOUR **Slots** that this process will need: `item_name`, `quantity`, `business_justification`, and `is_purchase_for_organization_acknowledgement`. 📖 *[Learn more about Slots](/docs/slots)*.
  ...
  <Step title="Build First Action Activity">
    Build the first **Action Activity** that fetch a list of historical purchase requests that the process will later compare against. 📖 *[Learn more about Activities](/docs/activities).*
  ...
  <Step title="Build Second Action Activity">
    Build the second **Action Activity**, which will utilize a Built-in Action that can generate an output according to a natural language prompt. 📖 *[Learn more about the Built-in Generate Text Action](/docs/built-in-actions#generate_text_action).*
  ...
  <Step title="Build Decision Policy">
    Build your first **Decision Policy**, which will execute after the already configured 2 Activity blocks. This policy will allow the Process to branch its execution depending on the `pr_classification` returned by the generate text Action Activity. 📖 *[Learn more about Decision Policies](/docs/control-flow).*
  ...
  <Step title="Create OpEx Action Activity">
    ...
    3. Fill out the following details for your Action Activity, which will connect your slots to the input(s) of the HTTP Action (and mapping the result of the Action back to the Conversational Process).
       ...
       <Note>
         Even though `is_purchase_for_organization_acknowledgement` is not technically needed to invoke the submit pr action, marking it as a required Slot for this Activity ensures that the AI Agent will ensure it fills the Slot before executing this Activity.
       </Note>
  </Step>
  <Step title="Create CapEx Content Activity">
  ...
  <Step title="Create Default Content Activity">
