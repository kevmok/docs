...
<CardGroup cols={2}>
  ...
  <Card title="Quickstart #5 Complete" icon="check">
    Completed [Quickstart #5: Notification Ambient Agent](/docs/webhook-triggers-quickstart-guide#/)
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Set up a Listener">
    Configure a webhook listener to receive expense approval requests.
  </Step>
  <Step title="Build the Compound Action">
    Create the logic to handle approvals with conditional routing.
  </Step>
  ...
  <Step title="Test the Workflow">
    Test the approval workflow using the Purple Suite testing tool.
  </Step>
</Steps>
...
<Steps>
  <Step title="Open Purple API Tool">
...
<Steps>
  ...
  <Step title="Navigate to Listener">
  ...
  <Step title="Publish Listener">
    Click "Publish" to save your listener.
  </Step>
  <Step title="Confirm Security">
...
<Steps>
  <Step title="Open Purple API Tool">
    Go back to the tab where your [Moveworks Purple API Tool](/docs/purple-api-tool) is open
  </Step>
  ...
  <Step title="Generate Webhook Payload">
    ...
    Make sure to tinker with the `amount` field to test different flows of your plugin.
  </Step>
  <Step title="Trigger Webhook">
